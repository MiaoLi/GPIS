...
option ipopt_options "max_iter=10000 tol=10e-8 acceptable_tol=10e-8 mu_strategy=adaptive bound_relax_factor=0.0001 dual_inf_tol=0.01 halt_on_ampl_error yes"; 
...
data data_grasp.dat;
#var kertesttrain{j in 1..nb_doigts, i in 1..datanb*4};
...
# kernel test train
#subject to constraints_kernel_test_train_1{j in 1..nb_doigts, i in 1..datanb}: kertesttrain[j,4*i-3] = 2*(sqrt
#(sum{k in 1..3}(pnew[j,k]-xtp[i,k])^2))^3-3*RKernel*(sum{k in 1..3}(pnew[j,k]-xtp[i,k])^2)+RKernel^3;

#subject to constraints_kernel_test_train_2{j in 1..nb_doigts, i in 1..datanb}: kertesttrain[j,4*i-2] = #2*3*sqrt(sum{k in 1..3}(pnew[j,k]-xtp[i,k])^2)*(pnew[j,1]-xtp[i,1])-3*RKernel*2*((pnew[j,1]-xtp[i,1]));

#subject to constraints_kernel_test_train_3{j in 1..nb_doigts, i in 1..datanb}: kertesttrain[j,4*i-1] = #2*3*sqrt(sum{k in 1..3}(pnew[j,k]-xtp[i,k])^2)*(pnew[j,2]-xtp[i,2])-3*RKernel*2*((pnew[j,2]-xtp[i,2]));

#subject to constraints_kernel_test_train_4{j in 1..nb_doigts, i in 1..datanb}: kertesttrain[j,4*i] = 2*3*sqrt(
#sum{k in 1..3}(pnew[j,k]-xtp[i,k])^2)*(pnew[j,3]-xtp[i,3])-3*RKernel*2*((pnew[j,3]-xtp[i,3]));

subject to constraint_point_uncertainty_1{j in 1..nb_doigts}:pvar[j] <=0.1;
...
display pnew >> point.txt;
display pvar >> point.txt;